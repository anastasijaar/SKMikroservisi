...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
	...
	@Autowired
	public JWTAuthorizationFilter(AuthenticationManager authManager, KartaRepository kartaRepo) {
		super(authManager);
	...
	private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request, String token) {
		...
		if (token != null) {
			...
			ResponseEntity<Boolean> response = restTemplate.exchange("http://localhost:8084/whoAmI", HttpMethod.GET, entity, Boolean.class);
			if(response.hasBody()) {
